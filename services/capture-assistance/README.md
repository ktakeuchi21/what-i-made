...
Required variables are documented in `.env.example`. In `AUTH_MODE=cognito`, API Gateway supplies validated JWT claims and the handler rechecks access-token use and the configured client ID. `RATE_LIMIT_TABLE` is mandatory and provides atomic, expiring per-subject limits; missing configuration fails closed. Configure reserved concurrency, budgets, and disabled model-invocation logging through the invitation stack. Package with `lambda.handler`.
