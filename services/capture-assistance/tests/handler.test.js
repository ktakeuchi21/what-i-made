...
const environment = { CAPTURE_ASSISTANCE_ENABLED: "true", COGNITO_CLIENT_ID: "client-123" };
const valid = { cleanedVoiceText: "I made adobo.", dishes: [], warnings: [] };
const event = (body) => ({
  rawPath: "/v1/parse-cook",
  requestContext: {
    http: { method: "POST" },
    requestId: "request-1",
...
test("requires trusted Cognito claims and the exact bounded request contract", async () => {
  const handler = createHandler({ provider: { parseCook: async () => valid }, allowRequest: async () => true, logger: { info() {} } }, environment);
  const unauthenticated = event({ transcript: "Adobo", voiceSegment: "Adobo", locale: "en-US" });
...
test("accepts only the API Gateway-validated Cognito access-token identity", async () => {
  const handler = createHandler({ provider: { parseCook: async () => valid }, allowRequest: async () => true, logger: { info() {} } }, environment);
  const request = event({ transcript: "I made adobo.", voiceSegment: "I made adobo.", locale: "en-US" });
...
test("returns validated provider output without logging private text", async () => {
  ...
  const handler = createHandler({ provider: { parseCook: async () => providerValue }, allowRequest: async () => true, logger: { info(value) { logs.push(value); } }, now: (() => { let value = 0; return () => value += 5; })() }, environment);
...
test("rejects cleaned output that is not limited to the new voice segment", async () => {
  ...
  const duplicated = createHandler({ provider: { parseCook: async () => ({ ...valid, cleanedVoiceText: `Earlier note ${voiceSegment}` }) }, allowRequest: async () => true, logger: { info() {} } }, environment);
...
test("kill switch and invalid provider output fail safely", async () => {
  ...
  const invalid = createHandler({ provider: { parseCook: async () => ({ ...valid, secret: "x" }) }, allowRequest: async () => true, logger: { info() {} } }, environment);
...
test("limits authenticated requests without retaining their text", async () => {
  ...
  const handler = createHandler({
    provider: { parseCook: async () => { calls += 1; return output; } },
    allowRequest: async () => { requests += 1; return requests <= 10; },
    logger: { info() {} },
    now: () => 100,
  }, environment);
