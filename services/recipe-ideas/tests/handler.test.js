...
function handlerFor(dependencies = {}, customEnvironment = environment) {
  return createHandler({ allowRequest: async () => true, ...dependencies }, customEnvironment);
}
...
test("all routes require trusted Cognito claims and honor the kill switch", async () => {
  const handler = handlerFor({ logger });
  const unauthenticated = event("POST", "/v1/recipes/search", { description: "soup" });
  delete unauthenticated.requestContext.authorizer;
  assert.equal((await handler(unauthenticated)).statusCode, 401);
  assert.equal((await handlerFor({ logger }, { ...environment, RECIPE_IDEAS_ENABLED: "false" })(event("POST", "/v1/recipes/search", { description: "soup" }))).statusCode, 503);
});

test("does not accept the retired shared-token authorization path", async () => {
  const request = event("POST", "/v1/recipes/search", { description: "soup" });
  delete request.requestContext.authorizer;
  request.headers = { authorization: "Bearer legacy_owner_token_that_is_long_enough" };
  const result = await handlerFor({ logger }, { ...environment, OWNER_TOKEN_SHA256: "a".repeat(64) })(request);
...
test("import safely fetches and normalizes a recipe with a signed image token", async () => {
  ...
  const handler = handlerFor({ fetchImpl: async () => response(document), lookup, logger, now: () => 1_000_000 });
...
test("import rejects pages without a complete recipe", async () => {
  const handler = handlerFor({ fetchImpl: async () => response("<html>No recipe</html>"), lookup, logger });
...
test("search is unavailable without a provider and sanitizes three sourced candidates", async () => {
  assert.equal((await handlerFor({ logger })(event("POST", "/v1/recipes/search", { description: "soup" }))).statusCode, 503);
  ...
  const result = await handlerFor({ searchProvider, logger })(event("POST", "/v1/recipes/search", { description: " noodle soup " }));
  assert.equal(result.statusCode, 200);
...
test("generate is unavailable without a provider and labels a valid draft", async () => {
  assert.equal((await handlerFor({ logger })(event("POST", "/v1/recipes/generate", { description: "soup" }))).statusCode, 503);
  ...
  const result = await handlerFor({ generateProvider, logger })(event("POST", "/v1/recipes/generate", { description: "soup" }));
  assert.equal(result.statusCode, 200);
...
test("image proxy requires auth and a valid signature, then returns bounded image bytes", async () => {
  ...
  const handler = handlerFor({ fetchImpl: async () => response("image-bytes", "image/jpeg"), lookup, logger, now });
...
test("rejects unknown fields, oversized bodies, unsafe URLs, and unknown routes", async () => {
  const handler = handlerFor({ fetchImpl: async () => response(""), lookup: async () => [{ address: "127.0.0.1", family: 4 }], logger });
