...
Dependency-free CommonJS code for a Node.js 20 AWS Lambda behind a Function URL or API Gateway. It supplies the network-only portion of **Ideas**; saved recipes and optimized images remain in the PWA's IndexedDB.

## Routes

Every route requires `Authorization: Bearer <owner-token>` and `RECIPE_IDEAS_ENABLED=true`.
...
- `OWNER_TOKEN_SHA256`: lowercase or uppercase SHA-256 hex of a 32–128 character URL-safe owner token.
