...
Conversational cleanup removes vocal fillers, context-free discourse markers, immediate repetitions, and abandoned false starts while preserving names, meaningful uses of “like,” “so,” and “oh,” quantities, negation, comparisons, uncertainty, and cooking details. The same versioned fixture contract covers the browser fallback and service sanitizer. Cleanup runs before local extraction and after structured service validation for the cleaned segment, dish names, notes, and ingredients. Dish-name extraction accepts explicit cooking/name phrases, recognized culinary names, and concise rating-linked names; a generic reaction or lone unknown opening word produces no dish suggestion. The client rewrites only the newly finalized voice segment, never pre-existing typed text. Service failure retains the verbatim segment and invokes the conservative local parser.
