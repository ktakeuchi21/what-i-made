...
The earlier owner-token Lambda and HTTPS endpoint are a superseded deployment. The invitation-only stack, Cognito-managed login, account-scoped archives, protected API routes, and owner migration are implemented and locally verified on the invitation branch. Next, follow the [owner-first rollout checklist](invitation-only-access/ROLLOUT_CHECKLIST.md): deploy identity and API authorization, disable the retired Function URLs, migrate and verify the owner's archive, publish the non-migration bundle, then create invitees and run the installed-iPhone acceptance pass.
