...
For a safe local simulation, open `http://localhost:4173/prototypes/capture-flow/?voice=fake&assist=fake&recipes=fake`. Add `&voiceFailure=connect` to exercise the recoverable connection-failure state. Smart assistance removes context-free filler from the finalized segment and structured text fields, fills only confident values, and leaves every suggestion editable. Dish names require explicit or recognizable culinary evidence, so an opening “oh” or unknown first word remains commentary rather than becoming the dish. Without `assist=fake` or a deployed endpoint, the conservative local parser applies the same cleanup contract.
